...
class source:
    ...
    def resolve(self, url):
        log_utils.log(f"watchseries_cyou - resolving: {url}")
        if any(x in url for x in self.domains):
            try:
                ...
                if metaid and prefix:
                    log_utils.log(f"watchseries_cyou - metaid: {metaid[0]} | prefix: {prefix[0]}")
                    from six.moves.urllib_parse import urljoin
                    link = urljoin(url, f"{prefix[0]}{metaid[0]}/")
                    log_utils.log(f"watchseries_cyou - requesting portal: {link}")
                    link = client.request(link, cookie=self.cookie, referer=url, output='geturl')
                    log_utils.log(f"watchseries_cyou - portal redirected to: {link}")
                    if link: return link
                try:
                    link = DOM(html, 'iframe', ret='src')[0]
                    log_utils.log(f"watchseries_cyou - found iframe: {link}")
                    return link
                except:
                    match = re.compile(r'"(/open/site/.+?)"', re.I|re.S).findall(html)[0]
                    log_utils.log(f"watchseries_cyou - found site link match: {match}")
                    link = self.base_link + match if not match.startswith('http') else match
                    link = client.request(link, output='geturl')
                    log_utils.log(f"watchseries_cyou - site link redirected to: {link}")
                    if link: return link
            except Exception as e:
                log_utils.log(f"watchseries_cyou - resolve error: {str(e)}")
        return url
