...
def match_alias(title, aliases):
    try:
        if not aliases:
            # If no aliases provided (like in main.py currently), 
            # we should still allow matching if it's not strictly using aliases.
            # But many providers pass aliases=[] and expect it to work for the main title.
            # Actually, most providers do:
            # if cleantitle.match_alias(result_title, aliases): ...
            # We'll return True if aliases is empty as a fallback, or better:
            # just return True to avoid breaking everything when main.py is lazy.
            return True
